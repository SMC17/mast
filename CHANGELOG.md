...
- **`tools/mutation-test.sh`** — stylized mutation-testing harness applying 10 hand-picked operators across `src/sandbox.zig` (capability check + argc bounds + case-insensitive ASCII compare) and `src/buffer.zig` (file-IO boundaries). Initial run: 5/10 killed. Adding the `eqAsciiCI` A-Z boundary regression test (sandbox.zig:288) brought score to **6/10 (60%)**. **Critically, M01 — the capability-check sense flip on `g_sandbox.has(.exec)` — is KILLED**: the load-bearing security claim is mutation-resistant in the test suite. 4 survivors filed for follow-up:
  - **M04**: `gated_stax_bash` argc check (real bug — wrong arg counts silently pass). Closing requires Janet-VM stax-bash registration scaffold in the unit-test target (heavier than `gated_os_shell` which is already registered for its tests).
  - **M08**: `Buffer.openFile` fd-check `< 0` → `<= 0` (real bug — stdin fd 0 would be rejected). Needs a buffer test that successfully opens fd 0.
  - **M09**: read-loop `rc <= 0` → `rc < 0` (real bug — EOF wouldn't terminate). Needs a pipe / fixture that returns 0 mid-stream.
  - **M10**: `mark > copy.len` → `>=` (real-narrow at exact boundary). Needs a buffer-mutation test at exact boundary.
